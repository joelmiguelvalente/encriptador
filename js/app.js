...
function crearElemento(etiqueta, objecto, agregar = 'appendChild', en = '') {
	...
	if(agregar !== '') {
		if(agregar === 'after') en.after(CreateNewElement)
		else if(agregar === 'appendChild') en.appendChild(CreateNewElement)
...
const redes = {
	...
	twitter: {
		url: 'https://twitter.com/JvalenteM92', 
...
const newUrl = location.href.split('/').slice(0,-1);
fetch(newUrl.join('/') + "/console.json")
.then(j => j.json())
.then(objecto => {
	const array = Object.values(objecto);
	array.map( items => {
		if(Array.isArray(items)) {
			items.map( item => {
				console.groupCollapsed(item.label);
					for(let liv = 0; liv < item.level.length; liv++) console.log(item.level[liv]);
				console.groupEnd();
			})
		} else {
			if(Array.isArray(items.txt)) {
				console.log(items.txt.join('\n').replaceAll('$1', '%c'), items.css[0], items.css[1], items.css[2])
			} else console.log(items.txt.replace('$1', '%c'), items.css)
		}
	})
})